(function (w, d) {
	...
	p.move = function(position) {
		...
		if (this.id !== board.memory.turn) {
			board.log("It's " + players[board.memory.turn].name + "'s turn");
			return;
		}
	...
	b.addPlayer = function(name) {
		...
		this.memory.playerCount++;
		return player;
	}
	
	b.nextPlayer = function() {
		if(this.memory.turn + 1 == this.memory.playerCount) {
			this.memory.turn = 0;
		} else {
			this.memory.turn++;
		}
